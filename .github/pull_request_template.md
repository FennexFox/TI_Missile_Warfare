## Summary

<!-- Briefly describe what this PR changes. -->

## Scope

<!-- What is intentionally included in this PR? -->

- 

## Non-goals

<!-- What is intentionally not handled here? -->

- 

## Implementation notes

<!-- Mention important design choices, scaffold boundaries, or tradeoffs. -->

- 

## Validation

<!-- Check all that apply and add command output when useful. -->

- [ ] `python tools/check_layout.py`
- [ ] Solution/project files load locally
- [ ] Core project builds locally
- [ ] Mod project builds locally with local Terra Invicta / UMM references
- [ ] Manual smoke test in game
- [ ] Not applicable: this PR is documentation/scaffold-only

## Reverse-engineering notes

<!-- For combat/modding PRs, document what game classes/methods were inspected or patched. -->

- Patched methods:
- Observed methods:
- Unknowns / follow-up investigation:

## Risk

<!-- Describe likely breakage points, especially patch targets and game-version coupling. -->

- 

## Rollback

<!-- How should this PR be reverted or disabled if it breaks? -->

- 

## Screenshots / logs

<!-- Optional. Add debug logs, screenshots, or allocation output if relevant. -->

