...
- Post-fix Cape St. George/Yayoi/Taiho log parse on the latest uploaded `Player.log` passed with `Verdict: OK`, zero MissileWarfare issues, zero same-team missile target snapshots, and `suspicious patterns: none`. The first two controlled candidates came from enemy allocator launcher `Yayoi` team `50` toward friendly `Verdun` team `47` and correctly classified as `wouldSkip` with `reason="allocatorLauncherOutsideSelectedTeam"`; the third candidate came from selected ship `Cape St. George` team `47` toward hostile `Taiho` team `50`, reached `gateResult="allowed"`, and produced one `appliedDecision` through `SelectSalvoTargetCommand.OnCommandExecute` with zero failed commands and zero scope violations.
